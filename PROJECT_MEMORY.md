...
- P2: Add smoke assertion coverage for malformed NDJSON line handling in `/api/answer/stream` with a corrupt chunk fixture.
- P2: Add deterministic fixture generator utility shared by smoke and perf harnesses to reduce data-shape drift.
- P2: Add focused tests for bootstrap-selected thread id sanitization (`trim`/dedupe/non-string drop).
- P3: Add changelog guard script validating that `Implemented` entries include commit hash tokens.
