...
- P2: Extend smoke with fixture-backed localStorage preload so `/search` operator assertions validate non-empty filtered sections with deterministic data.
- P2: Add smoke assertion for saved-search roundtrip (save, reload, run) under mock mode.
- P2: Add query-result parity tests proving top-k UI ordering matches full export ordering on tie scores and invalid timestamps.
- P2: Add quota-aware localStorage write guards for Unified Search state keys with explicit fallback notice.
